...
{
	LegoResult result;
	if ((result = ReadLE(p_storage, m_coordinates[0])) != SUCCESS) {
		return result;
	}
	if ((result = ReadLE(p_storage, m_coordinates[1])) != SUCCESS) {
		return result;
	}
	if ((result = ReadLE(p_storage, m_coordinates[2])) != SUCCESS) {
		return result;
	}
	return SUCCESS;
}
